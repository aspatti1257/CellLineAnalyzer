...
# training data greater than or equal to the coverage_threshold fits. If an explicit number of models to create is
# provided, it will make boolean subsets until that number is hit (determined by explicit_model_count).
# It can exit early if no more samples are covered after a certain number of attempts are made (determined by
# max_boolean_generation_attempts). Each boolean statement must cover between lower_bound and upper_bound percent of the
# complete training data. There will also be a fallback ElasticNet model which is trained on the entirety of the
# training data. For each model created, the R^2 score will be recorded fitted against the subset of samples which match
# the corresponding boolean phrase.
#
...
class RandomSubsetElasticNet(object):
